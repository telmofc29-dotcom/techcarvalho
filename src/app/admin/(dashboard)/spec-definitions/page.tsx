...
export default async function SpecDefinitionsPage({
  ...
  const [{ rows: specs, pageCount }, categories] = await Promise.all([
    listRowsPaginated("spec_definitions", { orderBy: "name", page, pageSize: ADMIN_PAGE_SIZE }),
    ...
    <div>
      ...
      {specs.length === 0 ? (
        ...
        <>
          ...
          <Pagination page={page} pageCount={pageCount} basePath="/admin/spec-definitions" />
        </>
      )}
    </div>
  );
